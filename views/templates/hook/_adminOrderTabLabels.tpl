...
<div class="dpd tab-pane" id="labels">
    <div class="clear"></div>
        <div class="row label">
            {if !$isInDb}
            <a href="{{$urlGenerateLabel}}" style="float: right" id="generate-label"  >
            ...
            <a href="{{$urlGenerateLabel}}" id="generate-label" target="_blank" >
                <button class="btn btn-default" style="float: right" id="showLabel"><i class="icon-download"> </i> {l s='Download DPD label' d='Modules.dpdbenelux'}</button>
            </a>


            <a href="{{$deleteGeneratedLabel}}" id="deletefromdatabase" >
                <button class="btn btn-danger" style="float: left" id="deleteLabelfromDatabase"><i class="icon-eraser"> </i> {l s='Delete  DPD label' d='Modules.dpdbenelux'}</button>
            </a>
        ...
        <div class="row" style="margin-top: 10px;">
            {if !$isReturnInDb}
                <a href="{{$urlGenerateReturnLabel}}" style="float: right" id="generate-label-retour">
                    <button class="btn btn-default"><i class="icon-rotate-right"></i> {l s='Print Retour Label' d='Modules.dpdbenelux'}</button>
                </a>
            ...
            {else}
                <a href="{{$urlGenerateReturnLabel}}" style="float: right" target="_blank">
                    <button class="btn btn-default"><i class="icon-rotate-right"></i> {l s='Download Retrour Label' d='Modules.dpdbenelux'}</button>
                </a>
...
<script type="text/javascript">
    function setAmountOfParcels(inputField, minButton, plusButton, link) {
        ...
        if (inputField.val() !== undefined && inputField.val() !== '') {
            ...
            link.click(function () {
                var newurl = url + '&parcel=' + inputField.val();
                $("#generateLabel").attr('href', newurl);
            });
        }
    }
